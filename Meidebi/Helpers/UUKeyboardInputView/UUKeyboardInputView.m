...
@interface UUKeyboardInputView ()<UITextViewDelegate>
{
    ...
    UUInputConfiger *_interiorConfiger;
    UIButton *btnBack;
...
+ (UUKeyboardInputView*)sharedView {
    ...
    dispatch_once(&once, ^ {
        ...
        [saveBtn setTitleColor:[UIColor colorWithHexString:@"#F16944"] forState:UIControlStateNormal];
        [saveBtn setTitleColor:[UIColor colorWithHexString:@"#F16944"] forState:UIControlStateSelected];
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    ...
    if(newLength <= _interiorConfiger.inputTextMax)
    {
        return YES;
    } else {
        NSUInteger emptySpace = _interiorConfiger.inputTextMax - (textView.text.length - range.length);
