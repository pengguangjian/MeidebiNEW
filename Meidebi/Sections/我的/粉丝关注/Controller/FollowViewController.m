...
<
FollowSubjectViewDelegate
>
@property (nonatomic, strong) FollowSubjectView *subjectView;
...
- (void)setupSubviews{
    _subjectView = [FollowSubjectView new];
...
- (void)loadData{
    [self.dataController requestFollowListInView:_subjectView Callback:^(NSError *error, BOOL state, NSString *describle) {
        if (state) {
...
#pragma mark - FollowSubjectViewDelegate
- (void)followSubjectViewDidClickAddFollowWithFollowID:(NSString *)userID didComplete:(void (^)(void))complete{
    [self.dataController requestAddFollwDataWithInView:_subjectView
                                                userid:userID
...
- (void)followSubjectViewDidClickCancelFollowWithFollowID:(NSString *)userID didComplete:(void (^)(void))complete{
    [self.dataController requestCancelFollwDataWithInView:_subjectView
                                                   userid:userID
                                                 callback:^(NSError *error, BOOL state, NSString *describle) {
        if (state) {
            complete();
