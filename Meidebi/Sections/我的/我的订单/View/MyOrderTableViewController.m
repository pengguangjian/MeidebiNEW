...
@interface MyOrderTableViewController ()<UIAlertViewDelegate,UIActionSheetDelegate>
{
...
//    MyOrderMainModel *model = arrdata[indexPath.row];
//    if(model.daigoutype.intValue == 1)
//    {
//        if([model.status intValue] == 2||[model.status intValue] == 3||[model.status intValue] == 5)
//        {
//            return 160;
//        }
//    }
...
//    [lbtuikuanstate setHidden:NO];
     ...
     */
    [lbtuikuanstate setText:[NSString nullToString:model.re_status]];
...
-(void)leftAction:(UIButton *)sender
{
    ...
    {///晒单赢优惠券
        ...
        NSDictionary *dicpush = @{@"id":_model.did,@"userkey" : [NSString nullToString:[MDB_UserDefault defaultInstance].usertoken]};
        [dataControllink requestMyInformYuanChuangKaPianInView:self.view dicpush:dicpush Callback:^(NSError *error, BOOL state, NSString *describle) {
            if(state)
            {
...
//    {
//        [MDB_UserDefault showNotifyHUDwithtext:@"请刷新页面" inView:self.view];
//    }
    NSDictionary *dicpush = @{@"page":[NSNumber numberWithInt:ipage],
                              ...
                              @"type":[NSString nullToString:_specialType]
                              };
...
//        [_dicAllSonValue setObject:arrdata forKey:_specialType];
    }];
