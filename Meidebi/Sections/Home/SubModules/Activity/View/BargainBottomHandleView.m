...
@interface BargainBottomHandleView ()
...
@property (nonatomic, strong) PelsonalHandleButton *remarkBtn;
@property (nonatomic, assign) BOOL isZan;
@end

@implementation BargainBottomHandleView
...
//    _collectBtn = collectBtn;
    ...
    [lineView1 mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerY.equalTo(likeBtn.mas_centerY);
        make.size.mas_equalTo(CGSizeMake(1, 10));
    }];
    lineView1.backgroundColor = [UIColor colorWithHexString:kButtonNormalStatueColor];
    
    PelsonalHandleButton *remarkBtn = [PelsonalHandleButton buttonWithType:UIButtonTypeCustom];
    ...
    remarkBtn.titleLabel.adjustsFontSizeToFitWidth = YES;
    remarkBtn.titleLabel.font = [UIFont systemFontOfSize:14.f];
    [remarkBtn setTitleColor:[UIColor colorWithHexString:kButtonNormalStatueColor]
                  forState:UIControlStateNormal];
    [remarkBtn setImage:[UIImage imageNamed:@"discount_comment_normal"] forState:UIControlStateNormal];
    [remarkBtn addTarget:self action:@selector(respondsToBtnEvent:) forControlEvents:UIControlEventTouchUpInside];
    _remarkBtn = remarkBtn;
    ...
    [commentHandleControl mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.bottom.equalTo(remarkBtn);
        make.right.equalTo(remarkBtn.mas_left).offset(-10);
    }];
...
- (void)bindViewData:(NSDictionary *)modelDict{
    if (!modelDict) return;
    [_likeBtn setTitle:modelDict[kBargainBottomLikeNumberKey] forState:UIControlStateNormal];
    [_readBtn setTitle:modelDict[kBargainBottomReadNumberKey] forState:UIControlStateNormal];
    [_collectBtn setTitle:modelDict[kBargainBottomCollectNumberKey] forState:UIControlStateNormal];
    [_remarkBtn setTitle:modelDict[kBargainBottomRemarkNumberKey] forState:UIControlStateNormal];
}

- (void)respondsToBtnEvent:(PelsonalHandleButton *)sender{
    BargainBottomHandleButtonType type;
    if (sender == _collectBtn) {
        type = BargainBottomHandleButtonTypeCollect;
    }else if (sender == _likeBtn){
        type = BargainBottomHandleButtonTypeLike;
    }else if (sender == _readBtn){
        type = BargainBottomHandleButtonTypeRead;
    }else if (sender == _remarkBtn){
        type = BargainBottomHandleButtonTypeRemark;
    }
    else{
        type = BargainBottomHandleButtonTypeRemark;
    }
...
- (void)updatePelsonalStatuesWithType:(BargainBottomUpdateViewType)type
    ...
    if (type == BargainBottomUpdateViewTypeZan) {
        if (_isZan) {
