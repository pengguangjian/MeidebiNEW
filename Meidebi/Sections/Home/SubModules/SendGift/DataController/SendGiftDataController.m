...
@interface SendGiftDataController()
@property (nonatomic, strong) UIView *targetView;
@property (nonatomic, assign) NSInteger page;

@end

@implementation SendGiftDataController

- (void)requestSendGiftListInView:(UIView *)view callback:(completeCallback)callback{
    _targetView = view;
    _page = 1;
...
- (void)loadDataWithDirection:(DragDirection)direction callback:(completeCallback)callback{
    ...
    NSDictionary *parameters=@{@"p":[NSString stringWithFormat:@"%@",@(_page)],
                               @"limit":@"30"};
    [HTTPManager sendGETRequestUrlToService:URL_SendGiftList
