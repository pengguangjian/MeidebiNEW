...
@property (nonatomic, strong) NJSegmentedControl *segmentedControl;
...
- (void)buildUI {
    ...
    for (int i = 0; i < number; i++) {
        ...
        [segmentTitles addObject:vc.title];
    ...
    _segmentedControl = [[NJSegmentedControl alloc] initWithFrame:CGRectMake(self.width/2-(kMainScreenW*.64)/2, self.tabView.height/2-46/2, kMainScreenW*.64, 46) items:segmentTitles andSelectionBlock:^(NSInteger segmentIndex) {
        [self selectTabWithIndex:segmentIndex animate:YES];
    }];
    [self.tabView addSubview:_segmentedControl];
    _segmentedControl.font = [UIFont systemFontOfSize:18];
    _segmentedControl.bgColor = [UIColor colorWithHexString:@"#FF925D"];
    _segmentedControl.textColor = [UIColor colorWithHexString:@"#FD7B0D"];
    _segmentedControl.cornerRadius = 6.f;
    
    //tabView self.tabBackgroundColor
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if (scrollView == self.bodyScrollView) {
        ...
        [self selectTabWithIndex:index animate:YES];
        [_segmentedControl setSelectedIndex:index];
