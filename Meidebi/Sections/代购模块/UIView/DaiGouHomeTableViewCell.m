...
@interface DaiGouHomeTableViewCell ()
{
    UIView *viewback;
...
@implementation DaiGouHomeTableViewCell
...
//        [lbshop setFrame:CGRectMake(lbzhiyou.right+2, imgvhead.bottom-17, 100*kScale, 17)];
    ...
    [viewsline setFrame:CGRectMake(lbshop.right+3, lbshop.top, 1, lbshop.height)];
    [viewback addSubview:viewsline];
    if(_model.name.length<1)
    {
        [viewsline setHidden:YES];
    }
    ///
    [lbtime setFrame:CGRectMake(viewsline.right+3, lbshop.top, 100, lbshop.height)];
    [lbtime setText:_model.addtime];
    ...
    [btaddgouwucar setBottom:viewback.height-15];
