...
@implementation RemarkStatusHelper
...
+ (NSRegularExpression *)regexEmoticon {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        regex = [NSRegularExpression regularExpressionWithPattern:@"\\[:[^ \\[\\]]+?\\]" options:kNilOptions error:NULL];///[:bibi_微笑眨眼] :xxx: {\\[:[^ \\[\\]]+?\\]} :+?:
    });
    return regex;
}
...
+ (NSDictionary *)emoticonDic {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSString *emoticonBundlePath = [[NSBundle mainBundle] pathForResource:@"emoticons" ofType:@"plist"];
...
+ (NSRegularExpression *)regexWelfareStrategy {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        regex = [NSRegularExpression regularExpressionWithPattern:@"[0-9]{1,}|(\\+[0-9]{1,})" options:kNilOptions error:NULL];
    });
    return regex;
}
...
//    }
    NSArray *emoticons = [self emoticonGroups];
    for (EmoticonGroup *group in emoticons) {
        for (Emoticon *emoticon in group.emoticons) {
            if (emoticon.png.length == 0) {
                if (emoticon.chs) dic[emoticon.chs] = emoticon.gif;
                if (emoticon.cht) dic[emoticon.cht] = emoticon.gif;
            }else{
                if (emoticon.chs) dic[emoticon.chs] = emoticon.png;
                if (emoticon.cht) dic[emoticon.cht] = emoticon.png;
            }
...
//    NSArray *folders = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:@"" error:nil];
...
+ (NSArray<EmoticonGroup *> *)emoticonGroups {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSString *emoticonPlistPath = [[NSBundle mainBundle] pathForResource:@"emoticons" ofType:@"plist"];
        ...
        for (int i = 0, max = (int)groups.count; i < max; i++) {
            ...
            NSString *path = [[NSBundle mainBundle] pathForResource:group.fileName ofType:@"plist"];
