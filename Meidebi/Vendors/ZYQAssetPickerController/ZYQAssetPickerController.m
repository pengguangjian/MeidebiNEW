...
//         baselineAdjustment:UIBaselineAdjustmentAlignCenters];
    ...
    [self.text drawInRect:CGRectMake(rect.size.width - titleSize.width - 2, (height - 12) / 2, kThumbnailLength, CGFLOAT_MAX)
           withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12],   NSParagraphStyleAttributeName:paragraphStyle,
                            NSBaselineOffsetAttributeName:[NSNumber numberWithInteger:1]
                            }];
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (_delegate!=nil&&[_delegate respondsToSelector:@selector(shouldTap)]) {
        if (![_delegate shouldTap]&&!_selected) {
            ...
            [[NSNotificationCenter defaultCenter]postNotificationName:@"showNotify" object:nil];
//            if ([[[[self superview] superview] superview] superview]) {
//                [MDB_UserDefault showNotifyHUDwithtext:@"一次最多添加10张图片" inView:[[[[self superview] superview] superview] superview] ];
//            }
...
- (id)init
{
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(shownotify) name:@"showNotify" object:nil];
...
-(void)shownotify{
    if (_maximumNumberOfSelection==10) {
        [MDB_UserDefault showNotifyHUDwithtext:@"一次只能添加10张图片" inView:self.view ];
    }else{
        [MDB_UserDefault showNotifyHUDwithtext:@"一共只能添加10张图片" inView:self.view ];
    }
...
//             [MDB_UserDefault showNotifyHUDwithtext:@"一次只能添加10张图片" inView:self.view ];
//        }else{
//            [MDB_UserDefault showNotifyHUDwithtext:@"一共只能添加15张图片" inView:self.view ];
//        }
//
//    }
